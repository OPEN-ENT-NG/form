    ...
    public void deleteUsers(JsonArray users) {
        ...
        if (users.size() > 0){
            ...
            if (userIds.size() > 0) {
                ...
                String query =
                        "DELETE FROM " + Formulaire.FORM_TABLE + " WHERE id IN (" +
                ...
                statementsBuilder.prepared("UPDATE " + Formulaire.DISTRIBUTION_TABLE + " SET responder_name = ? " +
                        "WHERE responder_id IN " + Sql.listPrepared(userIds), new JsonArray().add(Formulaire.DELETED_USER).addAll(userIds));
                ...
                statementsBuilder.prepared("UPDATE " + Formulaire.RESPONSE_FILE_TABLE + " SET filename = ? " +
                        ...
                        ")", new JsonArray().add(Formulaire.DELETED_USER_FILE).addAll(userIds));
