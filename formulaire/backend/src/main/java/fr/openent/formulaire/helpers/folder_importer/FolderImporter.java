package fr.openent.formulaire.helpers.folder_importer;
...
public class FolderImporter {
    ...
    public static class FolderImporterContext {
        ...
        public List<String> publicDocumentsIds = new ArrayList<>();

        public FolderImporterContext(String basePath, String userId, String userName) {
            this(basePath, userId, userName, null, true, null);
        }

        public FolderImporterContext(String basePath, String userId, String userName, List<String> publicDocumentsIds) {
            this(basePath, userId, userName, publicDocumentsIds, true, null);
        }

        public FolderImporterContext(String basePath, String userId, String userName, List<String> publicDocumentsIds, boolean doCommitToDocumentsCollection) {
            this(basePath, userId, userName, publicDocumentsIds, doCommitToDocumentsCollection, null);
        }

        public FolderImporterContext(String basePath, String userId, String userName, List<String> publicDocumentsIds, boolean doCommitToDocumentsCollection, String importIntoFolder) {
            this.basePath = basePath;
            this.userId = userId;
            this.userName = userName;
            this.doCommitToDocumentsCollection = doCommitToDocumentsCollection;
            this.publicDocumentsIds = publicDocumentsIds;
    ...
    private Future<JsonObject> commitToMongo(FolderImporterContext context, Future<FolderImporterContext> beforeCommit) {
        ...
        beforeCommit.onComplete(mappedContext -> {
            ...
            List<JsonObject> importList = new ArrayList<>(updatedDocs.size());

            for (int i = updatedDocs.size(); i-- > 0; )
                importList.add(self.sanitiseDocData(context, updatedDocs.getJsonObject(i)));

            MongoDbRepositoryEvents.importDocuments("documents", importList, "", false, result -> {
    ...
    private void moveOrphansToRoot(FolderImporterContext context) {
        ...
        Set<JsonObject> processedOrphans = new HashSet<>();
    ...
    private Future<FolderImporterContext> importFlatFiles(FolderImporterContext context) {
        ...
        this.fs.readDir(context.basePath, result -> {
            if (result.succeeded() == false) {
                ...
                for (String filePath : filesInDir) {
                    ...
                    String fileName = null, fileId1 = null;
                    do {
                        if (m.find() == false) {
                    ...
                    while (usedIds.contains(fileId1) == true); // This handles filenames like <UUID>_<UUID> that would cause conflicts
                    usedIds.add(fileId1);
                    ...
                    DocumentHelper.setId(fileDocument, fileId1);

                    if (context.publicDocumentsIds.contains(fileId1)) {
                        fileDocument.put("public", true);
                    }
                    else if (context.importIntoFolderId != null) {
     ...
     * @param context A fresh FolderImporterContext
     */
    public Future<JsonObject> importFoldersFlatFormat(FolderImporterContext context) {
        return this.commitToMongo(context, this.importFlatFiles(context).map(context));
    }
