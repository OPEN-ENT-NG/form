...
public class ImportExportService {
    ...
    public Future<JsonArray> importForms(JsonObject forms, String userId, String userName) {
        ...
        String query = "INSERT INTO " + FORM_TABLE + " (title, description, is_progress_bar_disabled, owner_id, " +
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            boolean progressBarDisabled = false;
            int progressBarIndex = fields.indexOf(IS_PROGRESS_BAR_DISABLED);
            if (progressBarIndex != -1 && progressBarIndex < entry.size()) {
                progressBarDisabled = entry.getBoolean(progressBarIndex);
            }

            JsonArray params = new JsonArray()
                    .add(entry.getString(fields.indexOf(TITLE)))
                    .add(entry.getString(fields.indexOf(DESCRIPTION)))
                    .add(progressBarDisabled)
                    .add(userId)
                    .add(userName)
