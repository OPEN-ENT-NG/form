...
export const getDndMove = (
  ...
  // S → S
  if (isActiveS && isOverS)
    return DndMove.R_TO_R;

  // S → QR
  if (isActiveS && isOverQR)
    return DndMove.R_TO_R;
  ...
  // QS → QS 
  if (isActiveQS && isOverQS)
    return DndMove.QS_TO_QS;

  // QS → ST (question d'une section A dans le top d'une section B)
  if (isActiveQS && isOverST && (activeElement as IQuestion).sectionId !== (overElement as ISection).id)
    return DndMove.QS_TO_QS

  // QS → SB (question d'une section A dans le bottom d'une section B)
  if (isActiveQS && isOverSB && (activeElement as IQuestion).sectionId !== (overElement as ISection).id)
    return DndMove.QS_TO_QS;
  ...
  // QS → QR
  if (isActiveQS && isOverQR)
    return DndMove.QS_TO_QR;
...
export const moveQStoQS = (elements: IFormElement[], active: IQuestion, overSection: ISection, targetSectionPos: number): IFormElement[] => {

  const activeSection = elements.find(e => isFormElementSection(e) && e.id === active.sectionId);
  if (!activeSection) return elements;
  
  const after = activeSection.position === overSection.position ? (active.sectionPosition! < targetSectionPos!) : (activeSection.position! < overSection.position!);
  ...
  if (!newSectionOver) return elements;
    ...
    const newElementsWithActiveQuestionAdded = addQuestionSection(newElementsWithoutActiveQuestion, active, targetSectionPos, newSectionOver as ISection, after);
