...
export default function useShare({
  ...
  const toggleRight = (shareRight: ShareRight, actionName: ShareRightActionDisplayName) => {
    ...
    const index: number = newShareRights.findIndex((x) => x.id === shareRight.id);
    const actionObject = state.shareRightActions.filter((shareRightAction) => shareRightAction.id === actionName)[0];

    const isActionRemoving: boolean =
      newShareRights[index].actions.findIndex((action) => action.id === actionName) > -1;
    ...
    if (isActionRemoving) {
      ...
      updatedActions = updatedActions.filter(
        (action) => !requiredActions.find((requiredAction) => requiredAction.id === action.id),
      );
      ...
      const requiredActions = state.shareRightActions.filter(
        (shareRightAction) =>
          ...
          !newShareRights[index].actions.find((action) => action.id === shareRightAction.id),
      );
      newShareRights[index] = {
        ...newShareRights[index],
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (typeof error === "string") toast.error(t("explorer.shared.status.error"));
  ...
  const currentIsAuthor = () => (resourceCreatorId === user?.userId ? true : false);
