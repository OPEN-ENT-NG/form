...
export const questionService: QuestionService = {
    ...
    async list(id: number, isForSection: boolean = false) : Promise<Question[]> {
        try {
            ...
            let data: IQuestionResponse[] = DataUtils.getData(await http.get(`/formulaire/${parentEntity}/${id}/questions`, { headers: { Accept: 'application/json;version=2'} }));
    ...
    async listAll(formId: number) : Promise<Question[]> {
        try {
            let data: IQuestionResponse[] = DataUtils.getData(await http.get(`/formulaire/forms/${formId}/questions/all`, { headers: { Accept: 'application/json;version=2'} }));
            return data.map((qr: IQuestionResponse) => new Question().build(qr));
        } catch (err) {
            notify.error(idiom.translate('formulaire.error.questionService.list'));
            throw err;
    ...
    async get(questionId: number) : Promise<any> {
        try {
            return DataUtils.getData(await http.get(`/formulaire/questions/${questionId}`, { headers: { Accept: 'application/json;version=2'} }));
        } catch (err) {
