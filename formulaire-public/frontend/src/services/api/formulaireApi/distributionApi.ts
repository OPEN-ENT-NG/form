...
import { FORMULAIRE_PUBLIC } from "~/core/constants.ts";
    ...
    getDistribution: builder.query<IDistribution[], void>({
      ...
      async onQueryStarted(_, { queryFulfilled }) {
        try {
          ...
          toast.error(t("formulaire.error.distributionService.list", { ns: FORMULAIRE_PUBLIC }));
        }
      },
    }),
    ...
    addDistribution: builder.mutation<IDistribution, number>({
      ...
      async onQueryStarted(_, { queryFulfilled }) {
        try {
          ...
          toast.error(t("formulaire.error.distributionService.create", { ns: FORMULAIRE_PUBLIC }));
        }
      },
    }),
  }),
